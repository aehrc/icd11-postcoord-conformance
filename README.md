...
A portable, CSV-driven conformance test suite for **ICD-11 postcoordinated cluster expressions**, evaluated against any FHIR terminology server that implements `CodeSystem/$validate-code`. Designed to run side-by-side against two backends — typically a candidate Ontoserver build and the WHO ICD-API FHIR endpoint — and diff per-case agreement.
...
onto.postman_environment.json            # baseUrl + backendLabel for Ontoserver
icdapi.postman_environment.json          # baseUrl + backendLabel for WHO ICD-API FHIR
run.sh                                   # newman runner + jq diff -> comparison.json
setup-ontoserver.sh                      # one-time fixture upload into local Ontoserver
...
# One-time: upload ICD-11 CodeSystems into a running Ontoserver.
# Reads ICD11toFHIR converter output from ~/code/ICD11toFHIR/target by default.
./setup-ontoserver.sh --onto http://localhost:8080/fhir

# Each run: compare candidate Ontoserver vs WHO ICD-API.
./run.sh --onto   http://localhost:8080/fhir \
         --icdapi http://localhost:9000/fhir
# Outputs: onto-results.json, icdapi-results.json, comparison.json
```

Both `--onto` and `--icdapi` are optional — each falls back to the `baseUrl` in the corresponding `*.postman_environment.json`. `./run.sh --help` for full usage.

The ICD-API endpoint is typically a local docker container the WHO publishes that exposes a FHIR endpoint.
...
2. Pick the **Ontoserver (local)** or **WHO ICD-API (FHIR)** environment.
...
Quick mental model: `expected` is the **suite's belief** (grounded in the WHO refguide); `onto` and `icdapi` are what each backend actually returns; `agree` is `onto === icdapi`. A row where both backends agree but disagree with `expected` is interesting — that's data, not a bug report.
...
- **Ontoserver fixtures must be complete enough** to exercise the rules — if you load `icd11-mms-minimal.json` you'll see most non-`BA41.0` stems return false with `NOT_IN_CS`. Use the full MMS linearization in `setup-ontoserver.sh`.
